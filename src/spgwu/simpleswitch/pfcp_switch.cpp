...
unsigned short udp_checksum(unsigned long init_sum, unsigned short *buf,int size){
  unsigned long sum = init_sum; 
  while(size>1){
    sum+=ntohs(*buf++);
    size-=2;
  }
...
{
    static int iCnt = 0;
    printf("signal : %d, The %d timer!!\n", sig, iCnt++);
    //exit(1);
}
void myHandler(union sigval sig)
{
  if(sum_byte!=0){
    static int count = 0;
    int realtime_rate = sum_byte/TIMER_INTERVAL;
    printf("\n[timer %d] /////////////////// %d B/s\n",count++ ,realtime_rate);
    sum_byte = 0;
  }
}
...
void timer(){
  ...
  // sigaddset(&sigIntHandler.sa_mask, SIGALRM);
  struct itimerval tv;
  printf("1\n");
...
/*
  ...
  evp.sigev_notify_function = myHandler; 
...
*/
  while (1) {
    ...
    if ((nread = read(sock_r, iov->msg_iov.iov_base, iov->msg_iov.iov_len)) >
      ...
      count++;
      struct iphdr* iph= (struct iphdr*)(iov->msg_iov.iov_base);
      struct in_addr src_ip, dst_ip;
      struct udphdr *udph=nullptr;
      src_ip.s_addr = iph->saddr;
      dst_ip.s_addr = iph->daddr;
      string proto_str("UNKNOWN UPlayer");
      if(iph->protocol==17){
        proto_str="UDP";
        udph = (struct udphdr*)((char*)(iov->msg_iov.iov_base) + 20);
      }else if(iph->protocol==6){
        proto_str="TCP";
      }else if(iph->protocol==1){
        proto_str="ICMP";
      }
      std::cout << "["<<count<<"] "<< inet_ntoa(src_ip)<< "--->";
      std::cout << inet_ntoa(dst_ip)<< " ";
      std::cout << "read " << nread << " bytes, " << " "<< proto_str <<std::endl;
      //printf("origin IPchecksum %x, UDPchecksum %x\n", ntohs(iph->check), ntohs(udph->check));
      iov->msg_iov.iov_len = nread;
      //std::cout << count % Options::getProportionControl()<<std::endl;  
      struct UDPPackage *send_buff = (struct UDPPackage*)((char*)(iov->msg_iov.iov_base) + 28);
      printf("DATA->FLAG  %u\n", send_buff->FLAG);
      if((count % Options::getProportionControl()&& send_buff->FLAG == FILE_DATA) || (send_buff->FLAG == FILE_RETRANSMISSION_RES) ||
        (send_buff->FLAG == FILE_INFO_RESPONSE)){ //
        //-->enb
        work_pool_->write(iov);
      }else{
        //-->AP
      /*  
        iph->saddr = inet_addr("12.1.1.12");
        iph->daddr = inet_addr("10.0.0.127");
        iph->check = 0;
        iph->check = htons(ip_checksum((unsigned short*)iph, sizeof(struct iphdr)));
        
        udph->dest = htons(5566);
        udph->check = 0;
        unsigned long  init_sum ;
        uint16_t *p_ip = (uint16_t *)(iov->msg_iov.iov_base);
        init_sum = ntohs(p_ip[6]) + ntohs(p_ip[7]) + ntohs(p_ip[8]) + ntohs(p_ip[9]) + 0x11 + ntohs(p_ip[12]);
        udph->check = htons(udp_checksum(init_sum, (unsigned short *)udph, ntohs(udph->len)));
        printf("changed IPchecksum %x, UDPchecksum %x\n", iph->check, udph->check);
        
        pfcp_switch_inst->send_to_core(reinterpret_cast<char* const>(iph), nread);
        */
        
        struct in_addr peer_addr;
        peer_addr.s_addr = inet_addr("10.0.0.59");
        spgwu_s1u_inst->send_udp(peer_addr, 5566, (char *)send_buff, nread - 28);
        free_pool_->write(iov);
        printf("udp send via AP complete.\n");
      }
      iov = nullptr;
    } else {
      ++errors;
            ...
            if (get_pfcp_session_by_up_seid(lseid, ssession)) {
              ...
              if ((*it_pdr)->get(far_id)) {
                ...
                if (ssession->get(far_id.far_id, sfar)) {
                  //printf("lseid %08lu far_id.far_id %08u num_bytes %ld\n", lseid, far_id.far_id, num_bytes);
