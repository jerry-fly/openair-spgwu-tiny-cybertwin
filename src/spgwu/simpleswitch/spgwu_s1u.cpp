...
void spgwu_s1u_task(void* args_p) {
  ...
  do {
    ...
    switch (msg->msg_type) {
      ...
      case HEALTH_PING:
        // if (itti_msg_timeout* to = dynamic_cast<itti_msg_timeout*>(msg)) {
        //   Logger::spgwu_s1u().info("HEALTH_PING event timer id %d", to->timer_id);
        // }
        break;
      
      // case THROUGHPUT_TIMER:
      //   if (itti_msg_tun_throughput* to = dynamic_cast<itti_msg_tun_throughput*>(msg)) {
      //     Logger::spgwu_s1u().info("THROUGHPUT_TIMER event timer id %d", to->timer_id);
      //   }
      //   break;
...
#define GTPU_MESSAGE_FLAGS_POS_IN_UDP_PAYLOAD 0
  ...
  if (gtpuh->version == 1) {//GTPU_V1
    // Do it fast, do not go throught handle_receive_gtpv1u_msg()
    if (gtpuh->message_type == GTPU_G_PDU) {//user GTP
      ...
      //printf("tunnel_id %08x endpoint %s received %ld bytes  ", tunnel_id, r_endpoint.toString().c_str(), bytes_transferred);
    ...
    printf("gtpu message_type %u", gtpuh->message_type);
...
void spgwu_s1u::send_udp(
  ...
  peer_sock_addr.sin_addr   = peer_addr;
  peer_sock_addr.sin_port   = htobe16(peer_udp_port);
  gtpu_l4_stack::send_udp(
      peer_sock_addr, send_buffer, num_bytes);
}
//------------------------------------------------------------------------------
void spgwu_s1u::send_g_pdu(
