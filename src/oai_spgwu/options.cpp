...
void Options::help() {
  std::cout << std::endl
            ...
            << "  -p, --proportioncontrol              control lte-wifi data flow proportion "
               "(val >0)."
            << std::endl;
}
...
bool Options::parseInputOptions(int argc, char** argv) {
  ...
  struct option long_options[] = {
      ...
      {"proportioncontrol", required_argument, NULL, 'p'}};

  // Loop on arguments
  while (1) {
    c = getopt_long(argc, argv, "horc:p::", long_options, &option_index);
    ...
    switch (c) {
      ...
      case 'p': {
        ...
        if(optarg!=nullptr)
          proportioncontrol = atoi(optarg);
        else
          proportioncontrol = UINT32_MAX;
        //options |= log_rot_file_log;
        //std::cout << "ProportionControl "<<proportioncontrol<< std::endl;
        break;
      }
